...
public sealed class InjectionEngine : IDisposable
{
    ...
    const uint INJECT_ACCESS = 0x0020 | 0x0010 | 0x0008 | 0x1000 | 0x0400;
    ...
    static void ResumeAndClose(List<IntPtr> handles)
    {
    ...
    public (int applied, int noOffset, int writeFail, int diskFallback) ApplyAll(
        ...
        FeatureSettings           feat)
    {
        ...
        try
        {
            if (useSuspend) threadHndles = SuspendThreads();
            ...
            foreach (var f in snapshot)
            {
                ...
                if (TryGetOffset(f.Name, offsets, out nint off))
                {
                    nint addr = BaseAddress + off;
                    SaveOriginalBytes(f.Name, addr, f.Type);
                    ok = WriteValueWithVerify(_hProc, addr, f.Value, f.Type);
                    if (!ok && feat.OffsetlessEnabled && scanner is not null)
                        ok = TryWriteOffsetlessWithOriginal(f, scanner);

                    if (ok) applied++; else writeFail++;
                }
                else if (feat.OffsetlessEnabled && scanner is not null
                      && TryWriteOffsetlessWithOriginal(f, scanner))
                {
                    applied++; ok = true;
                }
                else noOffset++;
            ...
            if (diskFlags?.Count > 0) diskFallback = WriteToDisk(diskFlags);
        }
        finally
        {
            if (threadHndles is not null) ResumeAndClose(threadHndles);
        }

        return (applied, noOffset, writeFail, diskFallback);
    }

    public bool ApplyOne(
        FlagEntry                f,
        Dictionary<string, nint> offsets,
        OffsetlessScanner?       scanner,
        ...
        bool                     diskFallback = false)
    {
        ...
        if (TryGetOffset(f.Name, offsets, out nint off))
        {
            nint addr = BaseAddress + off;
            SaveOriginalBytes(f.Name, addr, f.Type);
            bool ok = WriteValueWithVerify(_hProc, addr, f.Value, f.Type);
            if (!ok && offsetlessEnabled && scanner is not null)
                ok = TryWriteOffsetlessWithOriginal(f, scanner);
            if (!ok && diskFallback) WriteToDisk(new Dictionary<string,string>{{f.Name, f.Value}});
            return ok;
        }

        if (offsetlessEnabled && scanner is not null)
        {
            bool ok = TryWriteOffsetlessWithOriginal(f, scanner);
            if (!ok && diskFallback) WriteToDisk(new Dictionary<string,string>{{f.Name, f.Value}});
            return ok;
        }

        if (diskFallback) WriteToDisk(new Dictionary<string,string>{{f.Name, f.Value}});
        return false;
    }

    bool TryWriteOffsetlessWithOriginal(FlagEntry f, OffsetlessScanner scanner)
    {
        nint addr = scanner.FindValueAddress(_hProc, BaseAddress, f.Name);
        if (addr == 0) return false;

        SaveOriginalBytes(f.Name, addr, f.Type, replace: true);
        return WriteValueWithVerify(_hProc, addr, f.Value, f.Type);
    }
    ...
    public int Uninject()
    {
        ...
        try
        {
            foreach (var (_, (addr, original)) in _originalBytes)
            {
                try
                {
                    if (EnsureWritable(_hProc, (IntPtr)addr, original.Length, out bool restore, out uint oldProt))
                    {
    ...
    public bool UninjectOne(string flagName)
    {
        if (!IsAttached) return false;
    ...
    void SaveOriginalBytes(string name, nint addr, string type, bool replace = false)
    {
        if (!replace && _originalBytes.ContainsKey(name)) return;
    ...
    public bool WriteValueWithVerify(IntPtr h, nint addr, string val, string type)
    {
        try
        {
            if (type == "string") return WriteString(h, addr, val);

            byte[] buf = BuildBuffer(val, type);
            if (buf.Length == 0) return false;
            if (!EnsureWritable(h, (IntPtr)addr, buf.Length, out bool restore, out uint oldProt)) return false;
    ...
    bool WriteString(IntPtr h, nint valueInst, string val)
    {
        try
        {
            ...
            if (!EnsureWritable(h, (IntPtr)bufPtr, toWrite.Length, out bool restore, out uint oldProt)) return false;
            ...
            if (EnsureWritable(h, (IntPtr)(valueInst + 8), 8, out restore, out oldProt))
            {
    ...
    public bool WriteValue(IntPtr h, nint addr, string val, string type)
        => WriteValueWithVerify(h, addr, val, type);

    static readonly byte[] _bTrue  = { 1 };
    static readonly byte[] _bFalse = { 0 };

    static byte[] BuildBuffer(string val, string type)
    {
        switch (type)
        {
            case "bool":
    ...
    static bool EnsureWritable(IntPtr h, IntPtr addr, int size, out bool restore, out uint oldProt)
    {
    ...
    public static int WriteToDisk(Dictionary<string, string> flags)
    {
    ...
    public static bool TryGetOffset(string name, Dictionary<string, nint> offsets, out nint offset)
    {
